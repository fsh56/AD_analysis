...
#SBATCH --output=/scratch/sfeng56/ad_analysis/logs/mr_by_gene_%j.out
#SBATCH --error=/scratch/sfeng56/ad_analysis/logs/mr_by_gene_%j.err
#SBATCH --time=8:00:00
#SBATCH --mem=64G
...
R_SCRIPT="/scratch/sfeng56/draft/mr_by_gene_all.R"
...
# Combine all Median results
echo "Combining Median results..."
...
tail -n +2 "${OUTPUT_BASE}/all_Median_combined.csv" >> "${OUTPUT_BASE}/all_methods_combined.csv"
tail -n +2 "${OUTPUT_BASE}/all_cML_combined.csv" >> "${OUTPUT_BASE}/all_methods_combined.csv"

echo "Done! Results saved in: $OUTPUT_BASE"
echo "Summary files:"
echo "  - all_IVW_combined.csv"
echo "  - all_Egger_combined.csv"
echo "  - all_Median_combined.csv"
echo "  - all_cML_combined.csv"
