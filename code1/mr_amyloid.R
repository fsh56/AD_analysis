#!/usr/bin/env Rscript
# MR analysis by gene from clumped data - Amyloid specific
...
if (length(args) < 2) {
  stop("Usage: Rscript mr_amyloid.R <input_file.txt.gz> <output_dir>")
}
...
p_threshold <- sub(".*_p([0-9.e-]+)\\.txt.*", "\\1", basename(input_file))
...
for (i in seq_along(unique_genes)) {
  ...
  wm_result <- try(mr_median(mr_object, weighting = "weighted", iterations = 10000), silent = TRUE)
  ...
  if (exists("wm_result")) rm(wm_result)  # 新增
...
if (length(all_ivw_results) > 0) {
  ...
  output_file <- file.path(output_dir, paste0("amyloid_IVW_p", p_threshold, ".csv"))
...
if (length(all_egger_results) > 0) {
  ...
  output_file <- file.path(output_dir, paste0("amyloid_Egger_p", p_threshold, ".csv"))
...
if (length(all_wm_results) > 0) {
  ...
  output_file <- file.path(output_dir, paste0("amyloid_WeightedMedian_p", p_threshold, ".csv"))
  write.csv(wm_combined, output_file, row.names = FALSE)
  cat("Weighted Median results saved:", output_file, "\n")
  cat("  Total genes with Weighted Median results:", nrow(wm_combined), "\n")
}

# Clean up
rm(data, unique_genes, all_ivw_results, all_egger_results, all_wm_results)  # 更新
gc(verbose = FALSE)
