...
if (length(args) < 2) {
  cat("Usage: Rscript harmonize_gwas_etql.R <gwas_file_name> <tissue_name>\n")
  cat("Example: Rscript harmonize_gwas_etql.R amyloid.assoc.linear.gz Brain_Amygdala\n")
  quit(status = 1)
}
gwas_file = args[1]
tissue_name = args[2]
...
# read gwas data
cat("process GWAS data:", gwas_file, "...\n")
...
# loop thr chr1-22 and process eqtl data
for (chr in 1:22) {
