...
NumericVector gibbs_seso_uhp_only(int niter,
  ...
  double a_gamma = std::max(2.0, K / 2.0);
  double a_theta = a_gamma;
  double b_gamma = std::max(1e-6, (var(Gamma_hat) - pow(mean(s_hat_Gamma), 2)))*(a_gamma-1);
  double b_theta = std::max(1e-6, (var(gamma_hat) - pow(mean(s_hat_gamma), 2)))*(a_theta-1);
